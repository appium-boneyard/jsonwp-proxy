...
class JWProxy {
  ...
  async proxy (url, method, body = null) {
    ...
    log.info(`Proxying [${method} ${url || "/"}] to [${method} ${newUrl}] ` +
             (body ? `with body: ${_.trunc(JSON.stringify(body), 200)}` : 'with no body'));
    let res, resBody;
    try {
      res = await this.request(reqOpts);
      resBody = res.body;
      log.info(`Got response with status ${res.statusCode}: ${_.trunc(JSON.stringify(resBody), 200)}`);
